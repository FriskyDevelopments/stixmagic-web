 ...
 * When NEXT_PUBLIC_STIXMAGIC_MANIFEST_URL is set, fetches from that URL and validates
...
/**
 * Returns the pipeline manifest, either fetched from NEXT_PUBLIC_STIXMAGIC_MANIFEST_URL
...
export async function loadPipelineManifest(): Promise<PipelineManifest> {
  ...
  const url = process.env.NEXT_PUBLIC_STIXMAGIC_MANIFEST_URL;
