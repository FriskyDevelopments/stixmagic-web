...
## 2024-12-16 - Descriptive ARIA Labels for Format-Only Links
**Learning:** When implementing download links or buttons that lack visual context (e.g., buttons only displaying the file format like 'WEBM' or 'GIF' in the `GalleryCard`), screen reader users receive insufficient information about what they are downloading. A link that just announces 'WEBM' is ambiguous.
**Action:** Always add a descriptive `aria-label` (e.g., `Download {asset.name} as {format}`) to format-only download links to ensure accessibility and provide clear context for assistive technologies.
