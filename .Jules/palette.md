...
## 2024-12-16 - Handling Inactive Items and Dynamic Content Accessibly
**Learning:** For placeholders or non-interactive UI elements that represent future functionality (like "Coming Soon" steps), using the native `disabled` attribute removes them entirely from keyboard focus. This breaks discoverability for screen reader users. Additionally, when a user interaction immediately changes significant content elsewhere on the page (like a live preview region), that region remains silent to screen readers unless specifically marked.
**Action:** Replace `disabled` with `aria-disabled="true"` for items like "Coming Soon" tabs or steps so they remain in the natural keyboard tab/arrow order while semantically indicating they are inactive. Furthermore, whenever updating a distinct visual preview block based on a user's selection (e.g., selecting a mask), wrap the preview text in a container with `role="status"`, `aria-live="polite"`, and `aria-atomic="true"` so the newly updated context is naturally announced.
