...
## 2024-05-08 - Accessible Custom Radio Groups (Arrow Key Navigation)
**Learning:** Adding `role="radio"` and `role="radiogroup"` is not enough to make a custom radio group fully accessible. According to W3C ARIA Authoring Practices, a proper radio group requires a roving `tabIndex` (so only the selected element is in the tab sequence) and arrow key navigation (up/down/left/right) to switch focus and selection between options. Without this, standard `<button>` elements remain incorrectly in the tab sequence and don't match screen reader expectations for forms mode.
**Action:** Always implement a roving `tabIndex` (`tabIndex={selected ? 0 : -1}`) and an `onKeyDown` handler for arrow key navigation when building custom `role="radiogroup"` components out of standard buttons.
