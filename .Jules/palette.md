...
## 2024-12-16 - ARIA Disabled vs Native Disabled for Tooltips
**Learning:** Using the native HTML `disabled` attribute on buttons prevents them from receiving focus or firing pointer events, which also means any `title` tooltips won't show on hover. For states like "Coming soon" or disabled submit buttons where you want to explain *why* it's disabled, replacing `disabled` with `aria-disabled="true"`, safely guarding the `onClick` handler, and ensuring conditional CSS classes handle the inactive styling provides a much better UX. This allows the button to stay in the keyboard tab order and trigger hover tooltips.
**Action:** When a disabled state requires explanation (like a "Coming Soon" feature), prefer `aria-disabled="true"` with a `title` attribute and conditional click prevention over native `disabled`.
